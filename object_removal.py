...
def reduce_mask(mask, seam_idx):
    m, n = mask.shape
    output = np.zeros((m, n - 1))
    for row in range(m):
        output[row, : ] = np.delete(mask[row, : ], [seam_idx[row]])
    return output
...
def rotate_image(image, ccw, gray=False):
...
def object_removal(mask, in_height, in_width):
        print("object removal")
        rotation = False
        global out_image
        global img_counter
        r, c = np.where(mask > 0)
        object_height, object_width = np.amax(r) - np.amin(r) + 1, np.amax(c) - np.amin(c) + 1
        
        energy_map = calc_energy_map()
        energy_map[np.where(mask[:, :] > 0)] *= 0
        ...
        if(object_height < object_width):
            ...
            rotation = True
            mask = rotate_image(mask, 1, rotation)
        ...
        while(len(np.where(mask[:, :] > 0)[0]) > 0):
            ...
            img_counter+=1
            delete_seam(seam_idx)
            mask = reduce_mask(mask, seam_idx)
        
        print("object removal done.")
        if(rotation == True):
            num_pixels = in_height - out_image.shape[1]
        else:
            num_pixels = in_width - out_image.shape[1]
        
        print("seam insertion")
        seams_insertion(num_pixels, rotation)
        if(rotation):
            out_image = rotate_image(out_image, 0)
        print("seam insertion done.")

def template_matching(template, inp_image):
    print("Template matching")
    img_gray = cv2.cvtColor(inp_image, cv2.COLOR_BGR2GRAY)
    height, width, depth = inp_image.shape
    h, w = template.shape
    # print("Template shape:",template.shape,w,h)
    template = np.pad(template, ((0,height-h),(0,width-w)), 'constant')
    # print("Template shape:",template.shape,w,h)
    tem_fft = np.fft.fft2(template)
    tem_fft = np.fft.fftshift(tem_fft)
    plt.show()
    ...
    correl = np.multiply(img_fft, np.conj(tem_fft))
    correl = np.divide(correl, np.absolute(correl))
    mask_fft = np.fft.ifftshift(correl)
    mask_back = np.fft.ifft2(mask_fft)
    print(mask_back.shape)
    mask_back = np.real(mask_back)
    mask_back2 = np.divide(mask_back-np.min(mask_back), np.max(mask_back)-np.min(mask_back))*255
    print(np.max(mask_back2))
    plt.imshow(mask_back2, 'gray')
    plt.show()
    # mask_back = np.divide(mask_back, np.max(mask_back))
    
    
    for i in range(w*h):
        max_pt = np.where(mask_back==np.max(mask_back))
        y1 = max_pt[0][0]
        x1 = max_pt[1][0]
        mask_back[y1][x1]=0
        temp_img = inp_image.copy()
        cv2.rectangle(temp_img, (x1, y1), (x1+w, y1+h), (255,0,0), 2)
        plt.imshow(cv2.cvtColor(temp_img, cv2.COLOR_BGR2RGB))
        plt.show()
        print("Correct object? (y/n): "),
    ...
    if(choice=='n'):
        ...
        sys.exit(0)
    # cv2.imwrite('mask_back.jpg',mask_back)
    ...
    cv2.rectangle(rect_img,top_left, bottom_right, 1, -1)


    masked_data = cv2.bitwise_and(img_gray, img_gray, mask=rect_img)

    cv2.imwrite('match_mask.png',masked_data)
...
def main(input_file, template):
    ...
    mask = template_matching(template, in_image.astype(np.uint8)).astype(np.float64)

    print("starting...")
    object_removal(mask, in_height, in_width)
...
if __name__=='__main__':
    ...
    parser.add_argument('template', help="Object template")
    parser.add_argument('output_file', help="Output image")

    args = parser.parse_args()
    main(args.input_file, args.template)
